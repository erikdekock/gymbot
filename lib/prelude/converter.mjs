...
export function inferArchetype(prelude) {
  // Defense-in-depth: the canonical form is the short code (A1..A8). The
  // Prelude UI now emits short codes at source (app/onboarding/_state/
  // archetype.js), but normalize here too so any stale client or future
  // verbose source (e.g. "A2_returning_athlete") collapses to "A2" rather
  // than slipping through to the engine validator. Ticket 3714fef0…123d.
  if (prelude.detected_archetype) return prelude.detected_archetype.split('_')[0];
