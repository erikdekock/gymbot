...
import { getCouplingEntry, getExercise, getLayer5, getLadderForSlotRole } from "./kb-loader.mjs";
...
// SAFETY INVARIANT (PD §11 ground-2 premise): a clinical-progression goal
// must NEVER also be in the comeback-arc set. If it were, a fat-finger
// edit could bump E1/E2/.../F4 to phase_2 via the rule-3 retained-capacity
// gate (E1↔E7 adjacency is the obvious mis-edit risk). Module-load assert
// fails loud at import time so a broken constants edit can't ship.
{
  const overlap = [...CLINICAL_PROGRESSION_GOALS].filter((g) => COMEBACK_ARC_GOALS.has(g));
  if (overlap.length > 0) {
    throw new Error(
      `[engine] SAFETY INVARIANT VIOLATED: clinical-progression goal(s) also in comeback-arc set: ${overlap.join(",")}. Clinical goals are hard-locked to phase_1 at signup; comeback-arc membership would bump them to phase_2 via rule 3.`
    );
  }
}
 ...
 * exhausted by hard-filter failures. Carry-forward rules per L5 Spec §9 and
 * — for the CONTRA path — PD's DS-01 §3.5 three-tier ruling:
 *
 *   - CONTRA failure → three ordered tiers, each re-filtered against the FULL
 *     hard-filter chain (incl. CONTRA_MAP — defense in depth so a knee-contra
 *     never blind-swaps to a knee-unsafe id even if a tier lists one):
 *       Tier 1: slot's own L1 candidate pool, re-filtered           [EXISTING]
 *       Tier 2: substitution_alternatives of the failed exercise,
 *               re-filtered (similarity-biased, usually re-fails)   [EXISTING]
 *       Tier 3: slot_role cross-family ladder, KB-defined, ordered  [NEW §3.5]
 *     Slot-identity at Tier 3 is the movement-PATTERN (slot.slot_role), not
 *     the movement-family — so a knee-biased squat slot can reach knee-safe
 *     hinge/posterior content the squat family never lists.
 ...
 *   - SKILL_GATE / DIFFICULTY failure → unchanged.
 *
 * The slot's L1 candidate pool is the slot's `candidates[]` array minus
 * already-tried ids. Returns `{ exercise, path, resolved_tier }` on success or
 * `null` when every tier is exhausted (caller decides throw vs slot_dropped).
 */
function fallbackForSlot({ slot, failedExercise, failure, userProfile, triedIds }) {
  ...
  // Ordered tiers: { label, ids }. Walked top-down; first candidate that
  // clears runHardFilters wins. `path` records only the tiers actually
  // consulted, so applied_substitutions shows the exact walk that resolved
  // the slot (observability for KB Editor ladder calibration).
  let tiers;
  if (failure.failure_type === "contraindication") {
    const ladderIds = getLadderForSlotRole(slot.slot_role).filter((id) => !triedIds.has(id));
    tiers = [
      { label: "contra_failure → tier1 slot_pool (re-filtered against CONTRA_MAP)", ids: slotPool },
      { label: "contra_failure → tier2 sub_alt (re-filtered)", ids: tertiarySubAlts },
      { label: `contra_failure → tier3 cross-family ladder [slot_role="${slot.slot_role}"]`, ids: ladderIds },
    ];
  } else if (failure.failure_type === "equipment") {
    tiers = [
      { label: "equipment_failure → sub_alt", ids: tertiarySubAlts },
      { label: "equipment_failure → slot_pool", ids: slotPool },
    ];
  } else if (failure.failure_type === "skill_gate") {
    tiers = [
      { label: "skill_gate_failure → SKILL_GATES fallback", ids: failure.fallback_exercise_id ? [failure.fallback_exercise_id] : [] },
      { label: "skill_gate_failure → slot_pool", ids: slotPool },
    ];
  } else {
    tiers = [
      { label: "difficulty_failure → slot_pool", ids: slotPool },
      { label: "difficulty_failure → sub_alt", ids: tertiarySubAlts },
    ];
  }

  const path = [];
  for (const tier of tiers) {
    path.push(tier.label);
    for (const id of tier.ids) {
      ...
      if (r.pass) {
        return { exercise: ex, path, resolved_tier: tier.label };
      }
    }
  }
  return null;
 ...
 */
function resolveSlot(slot, userProfile, substitutions, alerts) {
  ...
  if (fb) {
    substitutions.push({
      ...
      failure_type: firstFailure.failure_type,
  ...
  // §4.4 escape hatch — slot_dropped. Reached ONLY when every fallback tier is
  // exhausted. For a CONTRA failure that means all three DS-01 §3.5 tiers
  // (slot_pool, sub_alt, cross-family ladder) came back empty after the
  // defense-in-depth re-filter — e.g. an edge contra (shoulder × bench slot)
  // where alpha ladder coverage is intentionally narrow, or a slot_role with
  // no ladder entry yet. We emit a logged slot_dropped and return null so the
  // session builder cleanly omits the slot; signup still completes. This must
  // be rare in practice. (Non-contra exhaustion has no Tier-3 escape and
  // remains a hard error — that path is a build-time bug, per the 12.3a brief.)
  if (firstFailure?.failure_type === "contraindication") {
    alerts.push({
      severity: "alert",
      message:
        `slot_dropped: ${slot.slot} (slot_role="${slot.slot_role}") — contra "${firstFailure.reason}" ` +
        `exhausted all three §3.5 tiers (slot_pool, sub_alt, cross-family ladder). ` +
        `tried=${[...triedIds].join(",")}. §4.4 escape hatch; primary compound otherwise never dropped.`,
    });
    return null;
  }
...
function buildSession({ template, variant, dayOfWeek, userProfile, archetype, substitutions, alerts }) {
  ...
  for (const slot of variant.slots) {
    const ex = resolveSlot(slot, userProfile, substitutions, alerts);
    if (!ex && slot.candidates.length > 0) {
      // resolveSlot already logged the structured slot_dropped alert (§4.4
      // escape hatch). Omit the slot from the session and carry on so signup
      // completes.
      continue;
    }
    const entry = {
